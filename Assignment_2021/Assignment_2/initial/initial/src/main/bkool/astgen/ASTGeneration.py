...
class ASTGeneration(BKOOLVisitor):
    ...
    def visitAttribute_decl(self, ctx: BKOOLParser.Attribute_declContext):
        ...
        isStatic = ctx.getChild(0).getText()
        isFinal = ctx.getChild(1).getText()
        attrList = []
        for x in idList:
            if isStatic != 'static' and isFinal == 'final':
                attrList.append(AttributeDecl(Static(), ConstDecl(x[0], varType, x[1])))
            elif isStatic == 'static' and isFinal == 'final':
    ...
    def visitAll_type(self, ctx: BKOOLParser.All_typeContext):
        return ClassType(ctx.ID().getText()) if ctx.ID() else self.visit(ctx.getChild(0))
    ...
    def visitLiteral(self, ctx: BKOOLParser.LiteralContext):
        if ctx.INT_LIT():
            ...
            return int(ctx.STRING_LIT().getText())
        elif ctx.BOOL_LIT():
