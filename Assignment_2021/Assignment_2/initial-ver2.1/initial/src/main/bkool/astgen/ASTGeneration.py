...
class ASTGeneration(BKOOLVisitor):
    ...
    def visitVar_list(self, ctx: BKOOLParser.Var_listContext):
        idlist = self.visit(ctx.attr_list()) if ctx.attr_list() else []
        type = self.visit(ctx.all_type())
        isFinal = True if ctx.FINAL() else False
        if not ctx.var_list():
            return [VarDecl(x[0], type, x[1]) for x in idlist] if not isFinal else [ConstDecl(x[0], type, x[1]) for x in idlist]
        else:
            return [VarDecl(x[0], type, x[1]) for x in idlist] + self.visit(ctx.var_list()) if not isFinal else [ConstDecl(x[0], type, x[1]) for x in idlist] + self.visit(ctx.var_list())
