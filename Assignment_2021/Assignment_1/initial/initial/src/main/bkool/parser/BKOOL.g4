...
memberlist: (STATIC? (attribute_decl | method_decl))*;

attribute_decl: FINAL? all_type attr_list SEMI;
...
array_type: (INT | FLOAT | BOOLEAN | STRING) LSB INT_LIT RSB;

attr_list: ID (EQ exp)? COMMA attr_list | ID (EQ exp)?;

method_decl: all_type? ID LB param_list? RB block_stmt;
...
array_lit: LP (literal (COMMA literal)*)? RP;
